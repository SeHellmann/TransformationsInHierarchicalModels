...
simulate_CPT_individ <- function(alpha, beta, gamma, delta, lambda, luce, n=NULL) {
  ...
  # Item-Loop, mixed gambles,gamble A
    ...
    z.a   <- pow(mixed_prospects.a[,4],gamma) + pow(mixed_prospects.a[,2],delta) 
    w.x.a <- pow(mixed_prospects.a[,4],gamma) / pow(z.a,(1/gamma)) 
    w.y.a <- pow(mixed_prospects.a[,2],delta) / pow(z.a,(1/delta)) 
    ...
    z.b   <- pow(mixed_prospects.b[,4],gamma) + pow(mixed_prospects.b[,2],delta) 
    w.x.b <- pow(mixed_prospects.b[,4],gamma) / pow(z.b,(1/gamma)) 
    w.y.b <- pow(mixed_prospects.b[,2],delta) / pow(z.b,(1/delta)) 
