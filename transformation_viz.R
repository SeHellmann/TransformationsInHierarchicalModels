...
means <- dat |> 
  ...
  ungroup() |> 
  pivot_longer(cols=Correct:Original, values_to = "mean", names_to = "Transformation") |> 
  mutate(dens = NA)
...
# plots --------------------------------------------------------------------
## theme ----------------------------------------------------------
source('custom_theme.R') # get Sebi's custom theme
...
xma_l <- 
  ...
  #geom_segment(dat=means |> filter(vars=='small' & Transformation=="Original"), aes(x = mean, xend = mean, y = 0, yend = dens, color=Transformation), linewidth=linewidths[2]) +
...
yma_l <- 
  ...
  geom_density(alpha = .5, linewidth=linewidths[2]) +
  geom_segment(dat=means |> filter(vars=='small' & Transformation=="Correct"), aes(x = mean, xend = mean, y = 0, yend = dens, color=Transformation), linewidth=linewidths[2]) +
...
yma_r <-  
  ...
  geom_segment(dat=means |> filter(vars=='large' & Transformation=="Correct"), aes(x = mean, xend = mean, y = 0, yend = dens, color=Transformation), linewidth=linewidths[2]) +
  scale_color_manual(values=two_colors_transformations)+
...
legend <- 
  ggplot(dat=means) +
  geom_segment(aes(x = mean, xend = mean, y = 0, yend = dens, color=Transformation), linewidth=linewidths[2]) +
  scale_color_manual(name="Transformation", values=two_colors_transformations) +
  theme_void()  
